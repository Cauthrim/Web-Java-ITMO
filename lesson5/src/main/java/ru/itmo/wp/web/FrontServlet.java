...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("lang") != null && response.getStatus() != HttpServletResponse.SC_NOT_FOUND) {
        ...
        for (Class<?> clazz = pageClass; clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method arrMethod : methods) {
                if (arrMethod.getName().equals(route.getAction())) {
        ...
        for (Method method : possibleMethods) {
            ...
            if (flag || Arrays.stream(method.getParameterTypes()).toList().isEmpty()) {
                if (pickedMethod != null) {
                    throw new ServletException("Ambiguous action method call [method=" + method + "]");
                }
                pickedMethod = method;
        ...
        if (sessionLang != null && !sessionLang.equals("en")) {
            template = newTemplate(pageClass.getSimpleName() + "_" + sessionLang + ".ftlh");
        }
        if (template == null) {
            template = newTemplate(pageClass.getSimpleName() + ".ftlh");
        }
        if (template == null) {
            throw new ServletException("Can't find template [templateName: " + pageClass.getSimpleName() + ".ftlh]");
        }
        ...
        try {
            ...
            if (sourceConfiguration == null) {
                throw new ServletException("Can't render template [pageClass="
                        + pageClass + ", action=" + pickedMethod + "]", e);
            }
        }
    }
