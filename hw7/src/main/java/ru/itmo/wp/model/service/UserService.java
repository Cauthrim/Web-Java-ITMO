...
public class UserService {
    ...
    public void validateRegistration(User user, String password) throws ValidationException {
        ...
        if (user.getEmail().chars().filter(ch -> ch == '@').count() != 1) {
            throw new ValidationException("Invalid email: a single @ is required");
        }
        if (user.getEmail().length() > 20) {
            throw new ValidationException("Email can't be longer than 20 characters");
        }
    ...
    public void switchAdmin(User user, Boolean switchFlag) {
        userRepository.switchAdmin(user, switchFlag);
    }
